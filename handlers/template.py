...
class Handler:
    ...
    async def get(self, request):
        ...
        if await a.is_logged():
            await a.init()
            return {'data':{'first_name':str(a.user.first_name), 'last_name':str(a.user.last_name), 'login': a.user.login, 'admin': a.user.admin, 'breadcrumb':[{'name':'Шаблоны', 'link':'/template'}]}}
        else:
